#!/usr/bin/env python3
"""Token usage and AUD cost report for the Open Chat Reviewer archive.

    cd ~/open-chat-reviewer
    set -a; source .chatreview/archive.env; set +a
    uv run python ~/token-report.py

Writes token-report.html plus two CSVs into the current directory.
...
# Validated categorical palette (dataviz six-checks, light and dark).
...
SELECT (ts AT TIME ZONE %s)::date                              AS day,
       coalesce(j #>> '{message,model}', '(unknown)')          AS model,
       coalesce(nullif(project, ''), '(unattributed)')         AS project,
       coalesce(session_ext, '(none)')                         AS session_ext,
       count(*)                                                                          AS messages,
       sum(coalesce((j #>> '{message,usage,input_tokens}')::bigint, 0))                  AS inp,
       sum(coalesce((j #>> '{message,usage,output_tokens}')::bigint, 0))                 AS out,
       sum(coalesce((j #>> '{message,usage,cache_creation,ephemeral_5m_input_tokens}')::bigint, 0)) AS cw5m,
       sum(coalesce((j #>> '{message,usage,cache_creation,ephemeral_1h_input_tokens}')::bigint, 0)) AS cw1h,
       sum(coalesce((j #>> '{message,usage,cache_creation_input_tokens}')::bigint, 0))   AS cw_total,
       sum(coalesce((j #>> '{message,usage,cache_read_input_tokens}')::bigint, 0))       AS read
FROM p
...
def demo_rows() -> list[dict]:
    rng = random.Random(7)
    projects = ["atlas", "beacon", "corvid", "driftwood", "ember", "(unattributed)"]
    models = ["claude-opus-5"] * 8 + ["claude-fable-5"] * 3 + ["claude-sonnet-5"]
    rows, start = [], dt.date(2026, 7, 3)
    for d in range(60):
        ...
        for _ in range(rng.randint(1, 4)):
            ...
            cw = rng.randint(1_000_000, 9_000_000)
            share = rng.random()
            rows.append({
                "day": day, "model": rng.choice(models),
                "project": rng.choice(projects),
                "session_ext": f"sess-{rng.randint(1000, 9999)}",
                "messages": n,
                "inp": rng.randint(500, 12_000),
                "out": n * rng.randint(300, 900),
                "cw5m": int(cw * share), "cw1h": int(cw * (1 - share)), "cw_total": cw,
                "read": rng.randint(40_000_000, 260_000_000),
            })
    return rows
...
def build(rows: list[dict]) -> str:
    if not rows:
        sys.exit("No usage rows found. Run token-probe.py first to see what is stored.")
...
def write_csvs(rows: list[dict]) -> None:
    ...
    for r in rows:
        c = usd_cost(r)
        daily[(r["day"], r["model"], r["project"])]["aud"] += aud(sum(c.values()))
        for k in ("inp", "out", "cw_total", "read"):
            daily[(r["day"], r["model"], r["project"])][k] += int(r[k] or 0)
        daily[(r["day"], r["model"], r["project"])]["messages"] += int(r["messages"])

    with open("token-report-daily.csv", "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["day", "model", "project", "messages", "input_tokens", "output_tokens",
                    "cache_write_tokens", "cache_read_tokens", "cost_aud"])
        for (d, m, p), v in sorted(daily.items()):
            w.writerow([d, m, p, int(v["messages"]), int(v["inp"]), int(v["out"]),
                        int(v["cw_total"]), int(v["read"]), f'{v["aud"]:.4f}'])

    sess = defaultdict(lambda: [0.0, 0, None, ""])
    for r in rows:
        s = sess[r["session_ext"]]
        s[0] += aud(sum(usd_cost(r).values()))
        s[1] += int(r["messages"])
        s[2] = r["day"] if s[2] is None else min(s[2], r["day"])
        s[3] = r["project"]
    with open("token-report-sessions.csv", "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["session", "project", "first_day", "messages", "cost_aud"])
        for k, v in sorted(sess.items(), key=lambda kv: -kv[1][0]):
            w.writerow([k, v[3], v[2], v[1], f"{v[0]:.4f}"])


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--demo", action="store_true", help="render from synthetic data, no database")
    ap.add_argument("--out", default="token-report.html")
    args = ap.parse_args()
    ...
    total = sum(aud(sum(usd_cost(r).values())) for r in rows)
    print(f"Wrote {args.out}")
    if not args.demo:
        print("Wrote token-report-daily.csv and token-report-sessions.csv")
    print(f"Total: {money(total)} AUD across {len(rows):,} aggregated rows")
